...
f1 = open("bl1_set_tot.txt", "r")
f2 = open("bl2_set_tot.txt", "r")
f3 = open("bl3_set_tot.txt", "r")
fi = open("bl_set_tot.txt", "w")
...
for i in range(0, len(c)):
	print>>fi, str(2*c[i]) + "		" + str(avg_th[i]) + "		" + str(thstd[i]) + "		" + str(avg_rt[i]) + "				" + str(rtstd[i])
